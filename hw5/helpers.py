...
def gen_recreate_commands(module, commands):
    ...
    Parameters
        ...
        - commands: commands needed to set up the recreate of a test in ipython3

    Returns
        - a string containing a message that can be used to get a test in the right state
    '''
...
def __check_type(actual, expected):
    '''
...
def __check_equals(actual, expected):
    '''
...
def check_none(actual, expected):
    ...
    if expected is None:
        msg = "\n\nThe function returned a value other than the expected value: None."
...
def check_result(actual, expected):
    ...
    msg = __check_type(actual, expected)
    if msg is not None:
        return "\n\n" + msg
    ...
    msg = __check_equals(actual, expected)
    if msg is not None:
        return "\n\n" + msg
    return None
...
def check_1d_iterable(actual, expected):
    ...
    for idx, (actual_val, expected_val) in enumerate(zip(actual, expected)):
        msg = __check_type(actual_val, expected_val)
        if msg is not None:
            return f"\n\nChecking a list.\nProblem found at index {idx}: {msg}"

        msg = __check_equals(actual_val, expected_val)
        if msg is not None:
            return f"\n\nChecking a list.\nProblem found at index {idx}: {msg}"
...
def check_2D_list_unstructured(actual, expected):
    ...
    for outer_idx, (actual_sub, expected_sub) in enumerate(zip(actual, expected)):
        # don't use check 1D iterable here, because we want better
        # error messages.
        
        # Check that actual_sub is a list
        if not isinstance(actual_sub, list):
            msg = f"\n\nChecking a list of lists. \nValue at index {outer_idx} is a value of {type(actual_sub)} when a list was expected."
            return msg
        ...
        if msg is not None:
            return f"\n\nChecking a list of lists.\nProblem with sublist at index {outer_idx} in the outer list: {msg}."


        # Check that all the elements of the sub list are the correct type, and equal to the expected
        for inner_idx, (actual_val, expected_val) in enumerate(zip(actual_sub, expected_sub)):
            msg = check_none(actual_val, expected_val)
            if expected_val is None:
                if msg is not None:
                    return f"\n\nChecking a list of lists.\n Problem found at at index ({outer_idx}, {inner_idx}): {msg}"
...
def check_dict(actual, expected):
    ...
    # Check values
    for key in expected:
        msg = __check_type(actual[key], expected[key])
        if msg is not None:
            return f'\n\nValue error at key "{key}"\n{msg}'

        msg = __check_equals(actual[key], expected[key])
        if msg is not None:
            return f'\n\nValue error at key "{key}"\n{msg}'
...
def check_2D_no_aliasing(actual, expected):
    ...
    """
    if len(expected) > 1:
        err_msg = ("\n\nThe rows of the mask must refer to separate lists.\n"
...
def check_2D_list_unmodified(param_name, before, after):
    ...
    msg = "\n\nYou modified the contents of the parameter {} (which is not allowed).\n"
